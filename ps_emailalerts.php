...
class Ps_EmailAlerts extends Module
{
    ...
    public function hookActionValidateOrder($params)
    {
        ...
        foreach ($params['order']->getCartRules() as $discount) {
            $items_table .=
                '<tr style="background-color:#EBECEE;">
					...
					<td style="padding:0.6em 0.4em; text-align:right;">-' . $contextLocale->formatPrice($discount['value_tax_excl'], $currency->iso_code) . '</td>
			</tr>';
        }
        ...
        $template_vars = [
            ...
            '{items}' => $items_table,
            '{total_paid}' => Tools::displayPrice($order->total_paid, null ),
            '{total_products}' => Tools::displayPrice($total_products, null),
            '{total_discounts}' => Tools::displayPrice($order->total_discounts, null),
            '{total_discounts_tax_excl1}' => Tools::displayPrice($order->total_discounts_tax_excl, null ),
            '{after_discounts_tax_excl1}' => Tools::displayPrice($order->total_products - $order->total_discounts_tax_excl, null),
            '{total_shipping}' => Tools::displayPrice($order->total_shipping, null ),
            '{total_shipping_tax_excl1}' => Tools::displayPrice(
                $order->total_shipping_tax_excl,
                null
            ),
            '{total_shipping_tax_incl}' => Tools::displayPrice($order->total_shipping_tax_incl, null),
            '{total_tax_paid}' => Tools::displayPrice(
                $order->total_paid_tax_incl - $order->total_paid_tax_excl,
                null
            ),
            '{total_wrapping}' => Tools::displayPrice($order->total_wrapping, null),
